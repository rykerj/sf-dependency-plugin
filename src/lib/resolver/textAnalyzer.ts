...
export function analyzeApexFile(filePath: string): TextDependency[] {
  ...
  while ((match = instantiationRegex.exec(source)) !== null) {
    const name = match[1];
    if (isCustomMetadataName(name)) continue;
  ...
  while ((match = staticCallRegex.exec(source)) !== null) {
    ...
    if (isCustomMetadataName(className)) continue;
    if (isApexKeyword(className)) continue;
    if (isSalesforceNamespace(className)) continue;
  ...
  // -------------------------------------------------------------------------
  // SOQL SELECT — custom field references only (__c / __r)
  // -------------------------------------------------------------------------
  const soqlSelectRegex = /\bSELECT\s+([\s\S]*?)\s+FROM\b/gi;
  ...
  // -------------------------------------------------------------------------
  // Dot-notation field access: someVar.MyField__c or someVar.MyRelation__r
  // -------------------------------------------------------------------------
  const dotFieldRegex = /\.([\w]+__(?:c|r))\b/g;
  while ((match = dotFieldRegex.exec(source)) !== null) {
    deps.push({ type: 'CustomField', apiName: match[1], referenceType: 'FieldAccess' });
  }

  // -------------------------------------------------------------------------
  // Custom object type declarations: MyObject__c varName = ...
  // -------------------------------------------------------------------------
  const objectTypeRegex = /\b([\w]+__c)\s+[a-z][\w]*/g;
